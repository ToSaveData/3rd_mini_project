    ...
    : QWidget{parent}, isSent(false)                                        //파일 서버 접속 여부를 false로 초기화
{
    ...
    id->setPlaceholderText(tr("id"));                                       //입력란이 비어있을 경우 id 자리임을 표시

    name = new QLineEdit(this);                                             //이름 입력란에 새로운 객체 대입
    name->setPlaceholderText(tr("name"));                                   //입력란이 비어있을 경우 이름 자리임을 표시
    ...
    /*채팅 로그 저장 기능*/
    save = new QPushButton(tr("save"), this);                               //save 버튼에 새로운 객체 대입
...
void ChatClient::connectButtonClicked()                                     //connectButton의 클릭 시그널을 처리하는 함수
{
    if(connectButton->text() == tr("Log In"))                               //text가 Log In이면
    {
        ...
        /*서버에 전송할 id와 이름을 @로 구분해서 저장*/
        QString data = name->text() + "@" + id->text();
        sendProtocol(Chat_Login, data.toStdString().data());                //연결된 서버에 Login 프로토콜 전송
...
void ChatClient::receiveData()                                              //서버에서 데이터를 받는 함수
{
    ...
    case Chat_logCheck:                                                     //프로토콜이 로그인 확인인 경우
        /*대기실에서 로그인 상태*/
        connectButton->setText(tr("Chat in"));                              //connectButton의 text를 변경
        id->setReadOnly(true);                                              //로그인한 id를 바꿀 수 없도록 설정
...
void ChatClient::sendData()                                                 //서버로 메세지를 보내는 함수
{
    ...
    QTreeWidgetItem* item = new QTreeWidgetItem();                          //로그를 기록할 아이템 생성
    item->setText(0, serverAddress->text());                                //아이피 주소
    item->setText(1, serverPort->text());                                   //포트 번호
    item->setText(2, id->text());                                           //고객 id
    item->setText(3, name->text());                                         //고객 성명
    item->setText(4, tr("me") + " : " + str);                               //메세지 내용
